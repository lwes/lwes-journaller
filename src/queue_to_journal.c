...
void* queue_to_journal(void* arg)
{
  ...
  while ( ! gbl_done ) {
    ...
    if ( gbl_rotate ) { // <gbl_rotate>
      ...
      if ( sink_ram_count++ >= 10000 ) { // time to rotate sink-ram ?
        if ( time(NULL) == sink_rotate ) // was last rotate in this second?
          sink_ram_count = 9000 ; // then we'll wait a bit before rotate_sink_ram()
        else {
          if ( jrn[jcurr].vtbl->close(&jrn[jcurr]) < 0 ) {
    ...
    if ( (que_read_ret = que.vtbl->read(&que, buf, bufsiz, &pending)) < 0 ) {
      if ( journaller_panic_mode == PANIC_IN_EFFECT ) { // queue is empty, is panic over?
        ...
        if ( journaller_panic_mode == PANIC_HURRYUP ) { // queue is empty, is hurry-up over?
          journaller_panic_mode = PANIC_NOT ; // panic is over
          LOG_INF("HURRYUP is over:  hi-burst: %lli packets, %lli bytes.\n",
              st.packets_in_burst_since_last_rotate, st.bytes_in_burst_since_last_rotate);
        }
      continue;
    }
    ...
    switch ( header_is_rotate(buf, &this_rotate) ) {
      ...
      case 1: { // Command::Rotate
                ...
                peer_correlate(buf, que_read_ret); //TODO: ?what did they mean by this? gdw.2006.11.28
                // fall through to write this Command::Rotate out before looping to actually rotate.
              }
...
fallthru:
              ...
              switch ( journaller_panic_mode ) {
                ...
                case PANIC_HURRYUP: // clear hurry-up mode if it has worked ...
                  if ( pending <= (arg_queue_max_cnt*arg_hurrydown_at)/100 ) {
                    LOG_INF("HURRYUP finish, pending=%i <= %i hi-burst: %lli packets, %lli bytes.\n",
                ...
                case PANIC_IN_EFFECT: // clear panic mode if it has worked ...
                  if ( pending <= 100 ) { // queue is empty, is panic over?
                    journaller_panic_mode = PANIC_SHUTDOWN ; // panic is over
                    LOG_INF("PANIC is over:  hi-burst: %lli packets, %lli bytes.\n",
                        st.packets_in_burst_since_last_rotate, st.bytes_in_burst_since_last_rotate);
                  } else
              ...
              if ( (jrn_write_ret = jrn[jcurr].vtbl->write(&jrn[jcurr], buf, que_read_ret)) 
                  != que_read_ret ) {
                LOG_ER("Journal write error -- attempted to write %d bytes, write returned %d.\n",
  ...
  while ( (que.vtbl->read(&que, buf, bufsiz, &pending) >= 0) && arg_queue_max_cnt-- ) ;
