...
void process_options(int argc, const char* argv[])
{
  static const struct poptOption options[] = {
    { "args",          0,  POPT_ARG_NONE,   &arg_args,           0, "Print command line arguements, then exit", 0 },
  ...
  if ( arg_journalls == NULL || ! arg_journalls[0] || ! strrchr(arg_journalls[0],'/') ) {
    LOG_ER("%s requires output pathnames in form '/path/to/archive/file.gz'\n", arg_basename);
    ++bad_options;
  }
  ...
  if ( 0 == arg_njournalls ) {
    static const char* default_arg_journalls[] = { "/tmp/all_events.log.gz", NULL };
  ...
  if ( arg_sink_ram != NULL ) {
    ...
    while ( --arg_njournalls >= 0 ) // copy original arg_journalls to arg_disk_journals
  ...
  if ( arg_version ) {
    printf(
        "The packet journaller is a program for recording LWES messages for\n"
        "later processing.\n"
        "\n"
        "It listens for UDP packets on an interface/IP address/port combination\n"
        "and writes them to an optionally compressed packet journal file.\n"
        "\n"
        "It adds a 22 bytes header with the packet size and sender information.\n"
        "See the README file for additional details.\n"
        "\n"
        "The journaller looks at the \"message type\" in each received packet and\n"
        "will rotate the logs if it matches \"Command::Rotate\".\n"
        "\n"
        );
