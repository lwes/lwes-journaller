...
int queue_factory(struct queue* this_queue)
{
  ...
  if ( strcmp(arg_queue_type, ARG_MSG) == 0 ) {
    if ( queue_msg_ctor(this_queue, arg_queue_name, arg_queue_max_sz, arg_queue_max_cnt) < 0 ) {
      LOG_ER("No SysV SHM support.\n");
      return -1;
    }
  } else if ( strcmp(arg_queue_type, ARG_MQ) == 0 ) {
    if ( queue_mqueue_ctor(this_queue, arg_queue_name, arg_queue_max_sz, arg_queue_max_cnt) < 0 ) {
    ...
    if ( module ) {
      ...
      lwes_journaller_queue_init_t init = (lwes_journaller_queue_init_t) dlsym(module, symname) ;
      if ( !dlerror() ) {
        if ( (*init)(this_queue, arg_queue_name, arg_queue_max_sz, arg_queue_max_cnt) < 0 ) {
      ...
      err = dlerror() ;
    }    LOG_ER("Unrecognized queue type '%s', try \"" ARG_MSG "\" or \"" ARG_MQ "\".\n%s\n",
        arg_queue_type, err);
    return -1;
  }
