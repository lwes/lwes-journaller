...
static void process_model(const char* argv[])
{
  ...
  while ( ! gbl_done ) {
    ...
    for ( i=0; i<arg_nreaders; ++i ) {
      if ( (-1 == xport_to_queue_pid[i]) || (waiting_pid == xport_to_queue_pid[i]) )
    ...
    if ( (waiting_pid < 0) && (errno == EINTR) ) {
      /* If we get a rotate, send it along to the process that writes journals. */
      if ( gbl_rotate ) {
        if ( -1 != queue_to_journal_pid ) {
          LOG_PROG("Sending SIGHUP to queue_to_journal[%d] to trigger log rotate.\n",
    ...
    if ( WIFSIGNALED(status) ) {
      ...
      log_msg(LOG_MASK_WARNING, __FILE__, __LINE__,
          "Our %s process exited (pid=%d) with signal %d, restarting.\n",
...
static void thread_model()
{
  ...
  if ( pthread_create(&queue_to_journal_tid, &m_pthread_attr, queue_to_journal, NULL) ) {
    PERROR("pthread_create(&queue_to_journal_tid, NULL, queue_to_journal, NULL)");
  }
  LOG_PROG("queue_to_journal_tid == %d\n", queue_to_journal_tid);

  for ( i=0; i<arg_nreaders; ++i ) {
    if ( pthread_create(&xport_to_queue_tid[i], &m_pthread_attr, xport_to_queue, NULL) ) {
      PERROR("pthread_create(&xport_to_queue_tid[i], NULL, xport_to_queue, NULL)");
    } {
      int schedpolicy ;
      pthread_attr_getschedpolicy((pthread_attr_t*)&m_pthread_attr, &schedpolicy) ;
      switch ( schedpolicy ) {
...
int main(int argc, const char* argv[])
{
  ...
  if ( is_multicast(arg_ip) ) {
    ...
    if ( arg_nreaders > 1 ) {
      LOG_WARN("Multiple reader threads can't be used with multi-cast addresses,"
          " will use a single thread instead.\n");
      arg_nreaders = 1;
    }
  } else {
    if ( arg_join_group ) {
